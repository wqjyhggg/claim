				...
				if (isset($this->mpdf->PageAnnots[$n])) {
					...
					foreach ($this->mpdf->PageAnnots[$n] as $key => $pl) {
						...
						if (isset($pl['opt']['c']) && $pl['opt']['c']) {
							$col = $pl['opt']['c'];
							if (substr($col,0,1) == 3 || substr($col,0,1) == 5) {
								$annotcolor .= sprintf('%.3F %.3F %.3F', ord(substr($col,1,1)) / 255, ord(substr($col,2,1)) / 255, ord(substr($col,3,1)) / 255);
							} elseif (substr($col,0,1) == 1) {
								$annotcolor .= sprintf('%.3F', ord(substr($col,1,1)) / 255);
							} elseif (substr($col,0,1) == 4 || substr($col,0,1) == 6) {
								$annotcolor .= sprintf('%.3F %.3F %.3F %.3F', ord(substr($col,1,1)) / 100, ord(substr($col,2,1)) / 100, ord(substr($col,3,1)) / 100, ord(substr($col,4,1)) / 100);
							} else {
								$annotcolor .= '1 1 0';
							}
						} else {
