...
class Mpdf implements \Psr\Log\LoggerAwareInterface
{
	...
	function PrintBodyBackgrounds()
	{
		...
		if ($this->bodyBackgroundColor) {
			...
			if (substr($this->bodyBackgroundColor,0,1) == 5) { // RGBa
				$s .= $this->SetAlpha(ord(substr($this->bodyBackgroundColor,4,1)) / 100, 'Normal', true, 'F') . "\n";
			} elseif (substr($this->bodyBackgroundColor,0,1) == 6) { // CMYKa
				$s .= $this->SetAlpha(ord(substr($this->bodyBackgroundColor,5,1)) / 100, 'Normal', true, 'F') . "\n";
			}
		...
		foreach ($this->pageBackgrounds as $bl => $pbs) {
			...
			foreach ($pbs as $pb) {
				...
				if ((!isset($pb['image_id']) && !isset($pb['gradient'])) || isset($pb['shadowonly'])) { // Background colour or boxshadow
					...
					if ($pb['col'] && substr($pb['col'],0,1) === '5') { // RGBa
						$s .= $this->SetAlpha(ord(substr($pb['col'],4,1)) / 100, 'Normal', true, 'F') . "\n";
					} elseif ($pb['col'] && substr($pb['col'],0,1) === '6') { // CMYKa
						$s .= $this->SetAlpha(ord(substr($pb['col'],5,1)) / 100, 'Normal', true, 'F') . "\n";
					}
	...
	function PrintTableBackgrounds($adjustmenty = 0)
	{
		...
		foreach ($this->tableBackgrounds as $bl => $pbs) {
			foreach ($pbs as $pb) {
				if ((!isset($pb['gradient']) || !$pb['gradient']) && (!isset($pb['image_id']) || !$pb['image_id'])) {
					$s .= 'q ' . $this->SetFColor($pb['col'], true) . "\n";
					if (substr($pb['col'],0,1) == 5) { // RGBa
						$s .= $this->SetAlpha(ord(substr($pb['col'],4,1)) / 100, 'Normal', true, 'F') . "\n";
					} elseif (substr($pb['col'],0,1) == 6) { // CMYKa
						$s .= $this->SetAlpha(ord(substr($pb['col'],5,1)) / 100, 'Normal', true, 'F') . "\n";
					}
	...
	function SetColor($col, $type = '')
	{
		...
		if (substr($col,0,1) == 3 || substr($col,0,1) == 5) { // RGB / RGBa
			$out = sprintf('%.3F %.3F %.3F rg', ord(substr($col,1,1)) / 255, ord(substr($col,2,1)) / 255, ord(substr($col,3,1)) / 255);
		} elseif (substr($col,0,1) == 1) { // GRAYSCALE
			$out = sprintf('%.3F g', ord(substr($col,1,1)) / 255);
		} elseif (substr($col,0,1) == 2) { // SPOT COLOR
			$out = sprintf('/CS%d cs %.3F scn', ord(substr($col,1,1)), ord(substr($col,2,1)) / 100);
		} elseif (substr($col,0,1) == 4 || substr($col,0,1) == 6) { // CMYK / CMYKa
			$out = sprintf('%.3F %.3F %.3F %.3F k', ord(substr($col,1,1)) / 100, ord(substr($col,2,1)) / 100, ord(substr($col,3,1)) / 100, ord(substr($col,4,1)) / 100);
		}
		if ($type == 'Draw') {
			$out = strtoupper($out);
		} // e.g. rg => RG
	...
	function GetJspacing($nc, $ns, $w, $inclCursive, &$cOTLdata)
	{
		...
		if ($kashida_present) {
			...
			for ($c = 0; $c < count($cOTLdata); $c++) {
				for ($i = 0; $i < strlen($cOTLdata[$c]['group']); $i++) {
					if (substr($cOTLdata[$c]['group'],$i,1) == 'S') {
						// Save from last word
		...
		if ($txt != '') {
			...
			if ($this->textshadow) {  // First to process is last in CSS comma separated shadows
				foreach ($this->textshadow as $ts) {
					...
					if (substr($ts['col'],0,1) == 5 && ord(substr($ts['col'],4,1)) < 100) { // RGBa
						$s .= $this->SetAlpha(ord(substr($ts['col'],4,1)) / 100, 'Normal', true, 'F') . "\n";
					} elseif (substr($ts['col'],0,1) == 6 && ord(substr($ts['col'],5,1)) < 100) { // CMYKa
						$s .= $this->SetAlpha(ord(substr($ts['col'],5,1)) / 100, 'Normal', true, 'F') . "\n";
					} elseif (substr($ts['col'],0,1) == 1 && substr($ts['col'],2,1) == 1 && ord(substr($ts['col'],3,1)) < 100) { // Gray
						$s .= $this->SetAlpha(ord(substr($ts['col'],3,1)) / 100, 'Normal', true, 'F') . "\n";
					}
	...
	function applyGPOSpdf($txt, $aix, $x, $y, $OTLdata, $textvar = 0)
	{
		...
		for ($i = 0; $i < count($unicode); $i++) {
			...
			if ($kashida) {
				...
				$nextbase = $i + 1;
				while (substr($OTLdata['group'],$nextbase,1) != 'C') {
					$nextbase++;
				}
	...
	function PaintDivBB($divider = '', $blockstate = 0, $blvl = 0)
	{
		...
		if (isset($this->blk[$blvl]['box_shadow']) && $this->blk[$blvl]['box_shadow'] && $h > 0) {
			foreach ($this->blk[$blvl]['box_shadow'] as $sh) {
				...
				if (substr($col1,0,1) == 5 && ord(substr($col1,4,1)) < 100) { // RGBa
					$shadow .= $this->SetAlpha(ord(substr($col1,4,1)) / 100, 'Normal', true, 'F') . "\n";
				} elseif (substr($col1,0,1) == 6 && ord(substr($col1,5,1)) < 100) { // CMYKa
					$shadow .= $this->SetAlpha(ord(substr($col1,5,1)) / 100, 'Normal', true, 'F') . "\n";
				} elseif (substr($col1,0,1) == 1 && substr($col1,2,1) == 1 && ord(substr($col1,3,1)) < 100) { // Gray
					$shadow .= $this->SetAlpha(ord(substr($col1,3,1)) / 100, 'Normal', true, 'F') . "\n";
				}
			...
			if (!$bSeparate && $buffer) {
				...
				for ($p = 0; $p < strlen($priority); $p++) {
					...
					if (isset($details[$side]['c']) && is_array($details[$side]['c'])) {
						if (substr($details[$side]['c'],0,1) == 3) {  // RGB
							$coldom = 10 - (((ord(substr($details[$side]['c'],1,1)) * 1.00) + (ord(substr($details[$side]['c'],2,1)) * 1.00) + (ord(substr($details[$side]['c'],3,1)) * 1.00)) / 76.5);
						}
					} // 10 black - 0 white
					if ($coldom) {
						$dom += $coldom;
