...
class Gradient
{
	...
	public function Gradient($x, $y, $w, $h, $type, $stops = [], $colorspace = 'RGB', $coords = '', $extend = '', $return = false, $is_mask = false)
	{
		...
		for ($i = 0; $i < count($stops); $i++) {
			// mPDF 5.3.74
			if ($colorspace === 'CMYK') {
				$this->mpdf->gradients[$n]['stops'][$i]['col'] = sprintf('%.3F %.3F %.3F %.3F', ord(substr($stops[$i]['col'],1,1)) / 100, ord(substr($stops[$i]['col'],2,1)) / 100, ord(substr($stops[$i]['col'],3,1)) / 100, ord(substr($stops[$i]['col'],4,1)) / 100);
			} elseif ($colorspace === 'Gray') {
				$this->mpdf->gradients[$n]['stops'][$i]['col'] = sprintf('%.3F', ord(substr($stops[$i]['col'],1,1)) / 255);
			} else {
				$this->mpdf->gradients[$n]['stops'][$i]['col'] = sprintf('%.3F %.3F %.3F', ord(substr($stops[$i]['col'],1,1)) / 255, ord(substr($stops[$i]['col'],2,1)) / 255, ord(substr($stops[$i]['col'],3,1)) / 255);
			}
	...
	private function parseMozLinearGradient($m, $repeat)
	{
		...
		for ($i = $startStops; $i < count($bgr); $i++) {
			...
			if (substr($col,0,1) == 1) {
				$g['colorspace'] = 'Gray';
			} elseif (substr($col,0,1) == 4 || substr($col,0,1) == 6) {
				$g['colorspace'] = 'CMYK';
			}
	...
	private function getStop($col, $el, $convertOffset = false)
	{
		...
		if (substr($col,0,1) == 5) {
			...
			$stop['opacity'] = ord(substr($col,4,1)) / 100;
		} elseif (substr($col,0,1) == 6) {
			// transparency from cmyka()
			$stop['opacity'] = ord(substr($col,5,1)) / 100;
		} elseif (substr($col,0,1) == 1 && substr($col,2,1) == 1) {
			// transparency converted from rgba or cmyka()
			$stop['opacity'] = ord(substr($col,3,1)) / 100;
		}
	...
	public function parseBackgroundGradient($bg)
	{
		...
		if ($count_bgr > 6) {
			...
			if (substr($cor,0,1) == 1) {
				$g['colorspace'] = 'Gray';
			} elseif (substr($cor,0,1) == 4 || substr($cor,0,1) == 6) {
				$g['colorspace'] = 'CMYK';
			}
			if ($cor) {
				$g['col'] = $cor;
