							...
							<div class="col-sm-12">
								<div class="eprovider-data">
									...
									<div class="row" style="border: 1px solid rgb(204, 204, 204); padding: 10px; margin-bottom: 9px">
										...
										<div class="col-sm-2">
											<label for="Postcode" class="col-sm-12">Postcode:</label>
											<?php echo form_input("eprovider[postcode][".$i."]", $value ["postcode"], array("class" => "form-control required", 'placeholder' => 'Postcode', "readonly" => "disabled")); ?>
										</div>
										<?php if($edit): ?>
							...
							<div class="col-sm-12">
								<div class="expenses-list">
									...
									<div class="row" style="border: 1px solid rgb(204, 204, 204); padding: 10px;">
										...
										<div class="col-sm-3">
											<?php echo form_label('Diagnosis:', 'service_description', array("class" => 'col-sm-12')); ?>
									...
									<div class="row" style="border: 1px solid rgb(204, 204, 204); padding: 10px;">
										...
										<!-- div class="col-sm-3">
											<?php echo form_label('Diagnosis:', 'diagnosis', array("class" => 'col-sm-12')); ?>
											<?php echo form_input("expenses_claimed[diagnosis][]", $value ['diagnosis'], array("class" => "form-control autocomplete_field")); ?>
										</div -->
										<div class="col-sm-3">
											<?php echo form_label('Diagnosis:', 'service_description', array("class" => 'col-sm-12')); ?>
										...
										<div class="col-sm-3">
											...
											<?php echo form_input("expenses_claimed[amount_claimed_org][]", $value ["amount_claimed_org"], array("class" => "form-control ")); ?>
											<?php echo form_hidden("expenses_claimed[amount_claimed][]", $value ["amount_claimed"]); ?>
										...
										<div class="col-sm-3">
											...
											<?php if (1 == $value["third_party_payee"]) { ?>
												<?php foreach ($bprovider_list as $val): ?>
												<option value="<?php echo $val['id']; ?>" <?php if ($val['id'] == $value["payee"]) { echo "selected"; } ?>><?php echo $val['name'] . " : " . $val['address'] . ", " . $val['province'] . " " . $val['country'] . " " . $val['postcode']; ?></option>
												<?php endforeach; ?>
											<?php } else { ?>
												<?php foreach ( $payees_list as $payee ) { ?>
												<?php if ($payee['payment_type'] == 'cheque') { ?> 
												<option value="<?php echo $payee['id']; ?>" <?php echo (($payee['id'] == $value['payee']) ? "Selected" : ""); ?>><?php echo $payee['payment_type'] . " : " . $payee['payee_name'] . " : " . $payee['address'] . ", " . $payee['province'] . " " . $payee['country'] . " " . $payee['postcode']; ?></option>
												<?php } else { ?> 
		...
		<div class="row"
			...
			<div class="col-sm-3">
				...
				<select name="expenses_claimed[payee][]" class="form-control expenses_payee_select required">
					...
					<?php foreach ( $payees_list as $payee ) { ?>
					<?php if ($payee['payment_type'] == 'cheque') { ?> 
					<option value="<?php echo $payee['id']; ?>"><?php echo $payee['payment_type'] . " : " . $payee['payee_name'] . " : " . $payee['address'] . ", " . $payee['province'] . " " . $payee['country'] . " " . $payee['postcode']; ?></option>
					<?php } else { ?> 
	...
	.on("click", ".add_payee", function(e){
   	...
   	.on("click", ".save-eprovider", function(e){
		...
		$.ajax({
			...
			method:"post",
			data:{claim_id:'<?php echo $claim_details["id"]; ?>',name:name,address:address,province:province,country:country,postcode:postcode},
			dataType:"json",
			success: function(data) {
				if (data.success) {
					myid.val(data.id);
