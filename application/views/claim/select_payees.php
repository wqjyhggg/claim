   ...
   foreach ($payees as $key => $value): $i++; ?>
      <div class="row"  style="border: 1px solid rgb(204, 204, 204); padding: 10px; margin-bottom: 9px">
         <div class="col-sm-12">
            ...
            <div class="col-sm-2">
               <?php 
               echo form_radio("payment_type_".$i, "email transfer",  ($value["payment_type"] == 'email'?TRUE:FALSE), array('class'=>'setpremium'));
               echo form_label('Email Transfer', 'Email Transfer');
               echo form_hidden('payees[id][]', $value['id']);
               ?>  
            </div>
         </div>
         <br/>
         <div class="col-sm-3 wire_transfer_section" <?php echo ($value["payment_type"] <> 'email'?'style="display:none"':''); ?>>
            <?php 
               echo form_label('Email Transfer:', 'Email Transfer', array("class"=>'col-sm-12'));
               echo form_input("payees[bank][]", $value["bank"], array("class"=>"form-control", 'placeholder'=>'Email Transfer'));
            ?>
         </div>
         ...
         <div class="col-sm-3 cheque_section" <?php echo ($value["payment_type"] == 'email'?'style="display:none"':''); ?>>
            <?php 
