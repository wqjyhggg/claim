	...
	public function get_mytask($para, $limit=30, $offset=0 ) {
		...
		if (isset($para['finished'])) {
			if ($para['type'] == "CLAIM") {
				...
				$sql = "SELECT SQL_CALC_FOUND_ROWS t.id,t.user_id,t.item_id,task_no,t.category,t.due_date,t.due_time,t.completion_date,t.type, 0 as priority,t.created_by,t.user_type,c.status2 as status,t.created,t.finished,t.notes,t.logs FROM mytask t LEFT JOIN `claim` c ON (t.item_id=c.id AND t.type='CLAIM') WHERE (c.status2!='Closed' AND c.status2!='Denied')";
			} else {
