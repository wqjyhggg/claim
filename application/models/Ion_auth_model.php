...
class Ion_auth_model extends CI_Model {
	...
	public function login($identity, $password, $remember = FALSE) {
		...
		$user = $this->users_model->get_by_email($identity);
	...
	public function users($groups = NULL, $search = "") {
		...
		if ($search)
			$this->_ion_where = array(
					"(concat_ws(' ', " . $this->tables['users'] . ".first_name, " . $this->tables['users'] . ".last_name) like '%$search%' OR " . $this->tables['users'] . ".email like '%$search%') " 
			);
	...
	public function get_users_groups($id = FALSE) {
		...
		return $this->users_model->get_users_groups($id);
	...
	public function update_last_login($id) {
		...
		$para = array('id' => $id, 'last_login' => time());
		$this->users_model->save($para);
		return $this->db->affected_rows() == 1;
	}
