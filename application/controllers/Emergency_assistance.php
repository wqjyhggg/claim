...
class Emergency_assistance extends CI_Controller {
  ...
  public function doc_upload() {
    ...
    if ($this->upload->do_upload('doc_file')) {
      ...
      $para['url'] = base_url("/assets/uploads/".$path.$para['filename']);
  ...
  public function del_doc_upload($id, $case_id) {
    ...
    if ($rc = $this->case_file_model->get_by_id($id)) {
      $filepath = UPLOADFULLPATH."case_docs/".$case_id."/".$rc["filename"];
      @unlink($filepath);
      $this->case_file_model->delete_by_id($id);
      redirect('emergency_assistance/edit_case/'.$case_id, 'refresh');
    } else {
      return show_error("Sorry, Upload file error");
		}
	}
