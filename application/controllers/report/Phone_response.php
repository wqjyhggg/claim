...
class Phone_response extends CI_Controller {
	...
	public function index() {
		if (!$this->ion_auth->logged_in()) {
			...
			$records = array();
			if ($rts = $this->phone_model->phone_response($para)) {
			...
			$this->data['export_url'] = site_url('report/phone_response/export');
			...
			$this->template->write_view('content', 'report/phone_response', $this->data);
	...
	public function export() {
		if (!$this->ion_auth->logged_in()) {
			...
			header('Content-Disposition: attachment; filename=Phone_response.csv');
			...
			foreach($records as $key => $value ) { 
				...
				fputcsv($output, array($key,
						...
						$value['answers'],
						$value['abandons'],
						empty($value['answers']) ? '0%' : number_format($value['less10'] * 100 / $value['answers'], 2) . "%",
						empty($value['answers']) ? '0%' : number_format($value['less20'] * 100 / $value['answers'], 2) . "%",
						empty($value['answers']) ? '0%' : number_format($value['less30'] * 100 / $value['answers'], 2) . "%",
						empty($value['answers']) ? '0%' : number_format($value['less40'] * 100 / $value['answers'], 2) . "%",
						empty($value['answers']) ? '0%' : number_format($value['less50'] * 100 / $value['answers'], 2) . "%",
						empty($value['answers']) ? '0%' : '100%',
			...
			fputcsv($output, array(
					...
					$t_answers,
					$t_abandons,
					empty($t_answers) ? '0%' : number_format($t_less10 * 100 / $t_answers, 2) . "%",
					empty($t_answers) ? '0%' : number_format($t_less20 * 100 / $t_answers, 2) . "%",
					empty($t_answers) ? '0%' : number_format($t_less30 * 100 / $t_answers, 2) . "%",
					empty($t_answers) ? '0%' : number_format($t_less40 * 100 / $t_answers, 2) . "%",
					empty($t_answers) ? '0%' : number_format($t_less50 * 100 / $t_answers, 2) . "%",
					empty($t_answers) ? '0%' : '100%',
