...
class Cron extends CI_Controller
{
    ...
    foreach ($cdir as $value) {
      if (!in_array($value, array(".", ".."))) {
        if (is_dir($dir . DIRECTORY_SEPARATOR . $value)) {
          $result = array_merge($result, $this->dirToArray($dir . DIRECTORY_SEPARATOR . $value));
        } else {
  ...
  public function doc_to_s3()
  {
    $this->valid();

    $this->load->model('phone_model');

    $dir = FCPATH."assets/uploads/claim_files";
    if ($dirlist = $this->dirToArray($dir)) {
      $folderlen = strlen($dir);
      foreach ($dirlist as $filename) {
        $key = "claim_files".substr($filename, $folderlen);
        if ($this->phone_model->save_s3_file($filename, $key)) {
          echo "Uploaded file : ".$key."\n";
          $bname = basename($key);
          $id = basename(dirname($key));

          $find = 0;
          if ($rt = $this->db->like("files", $bname)->get('claim')->result_array()) {
            foreach ($rt as $rc) {
              if ($id != $rc["id"]) {
                continue;
              }
              $nameArr = explode(',', $rc['files']);
              foreach ($nameArr as $idx => $name) {
                if (strpos($name, $bname) !== false) {
                  $find = 1;
                  if (substr($name, 0, 4) != "http") {
                    $nameArr[$idx] = base_url("phone/file2/" . $key);
                  }
                  break;
                }
              }
          ...
          if ($find) {
            $this->db->set("files",implode(",", $nameArr))->where("id", $id)->update('claim');
          } else {
            echo "Can not find related claim for Uploaded file : ".$key."\n"; 
          }
          if (unlink($filename)) {
    ...
    $dir = FCPATH."assets/uploads/eclaim_files";
    if ($dirlist = $this->dirToArray($dir)) {
      $folderlen = strlen($dir);
      foreach ($dirlist as $filename) {
        $key = "eclaim_files".substr($filename, $folderlen);
        if ($this->phone_model->save_s3_file($filename, $key)) {
          echo "Uploaded file : ".$key."\n";
          $bname = basename($key);
          ...
          if ($find) {
            $this->db->set("name", $rc['name'])->where("id", $rc['id'])->update('eclaim_file');
          } else {
            echo "Can not find related eclaim for Uploaded file : ".$key."\n"; 
          }
          if (unlink($filename)) {
            echo "Delete ".$filename."\n";
          } else {
    ...
    $dir = FCPATH."assets/uploads/intake_forms";
    if ($dirlist = $this->dirToArray($dir)) {
      $folderlen = strlen($dir);
      foreach ($dirlist as $filename) {
        $key = "intake_forms".substr($filename, $folderlen);
        if ($this->phone_model->save_s3_file($filename, $key)) {
          echo "Uploaded file : ".$key."\n";
          $bname = basename($key);
          $id = basename(dirname($key));
          ...
          $find = 0;
          if ($rc = $this->db->where("id", $id)->get('intake_form')->row_array()) {
            $nameArr = explode(',', $rc['docs']);
            foreach ($nameArr as $idx => $name) {
              if (strpos($name, $bname) !== false) {
                $find = 1;
                if (substr($name, 0, 4) != "http") {
          ...
          if ($find) {
            $this->db->set("docs",implode(",", $nameArr))->where("id", $id)->update('intake_form');
          } else {
            echo "Can not find related intake_form for Uploaded file : ".$key."\n"; 
          }
          if (unlink($filename)) {
            echo "Delete ".$filename."\n";
          } else {
...
>>>>>>> 309a818a1a3ff13c4ea20e4b923e0fc2654dbbb1
}
