...
*/
	...
	public function block_check() {
		...
		$firstname = strtolower($this->input->get_post('firstname'));
		$lastname = strtolower($this->input->get_post('lastname'));
		$birthday = $this->input->get_post('birth');
		
		$rt = $this->block_list_model->check_list_name($firstname, $lastname, $birthday);
		$rdata['isblocked'] = (empty($rt) || ($rt["status"] != 2)) ? 0 : 1;
		$rdata['status'] = Api_model::STATUS_OK;
		$rdata['message'] = '';
